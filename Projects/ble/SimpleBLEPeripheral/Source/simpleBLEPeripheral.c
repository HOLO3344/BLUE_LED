...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  // Setup the SimpleProfile Characteristic Values
  {
    ...
    uint8 charValue9[SIMPLEPROFILE_CHAR9_LEN] = {0};
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
   ...
   if ( events & SBP_SET_PWM_EVT )
  {
   ...
   if ( events & SBP_SET_START_T3_EVT )
  {
    start_t3_task();
    return (events ^ SBP_SET_START_T3_EVT);
  } 
  
  if ( events & SBP_SET_STOP_T3_EVT )
  {
    stop_t3_task();
    return (events ^ SBP_SET_STOP_T3_EVT);
  }
...
static void simpleProfileChangeCB( uint8 paramID )
{
  ...
  switch( paramID )
  {
    ...
    case SIMPLEPROFILE_CHAR6:
      ...
      Uart_Print("SIMPLEPROFILE_CHAR6\r\n", sizeof("SIMPLEPROFILE_CHAR6\r\n"));
      break;
    ...
    case SIMPLEPROFILE_CHAR7:
      ...
      if(newValue[1] < 20)
      {
        get_task_colour_def(newValue[1]);
        osal_set_event(simpleBLEPeripheral_TaskID, SBP_SET_PWM_EVT);
      }
      Uart_Print(newValue, SIMPLEPROFILE_CHAR7_LEN);
      break;

    case SIMPLEPROFILE_CHAR8:
      SimpleProfile_GetParameter( SIMPLEPROFILE_CHAR8, newValue );  
      
      u8Index = newValue[1] & 0x0f;    
      UserTaskColourTabSize = newValue[1] >> 4;
      u8Index -= 1;
      u8Index <<= 2;
      osal_memcpy(&UserTaskColourTab[u8Index], &newValue[2], (sizeof(tempTASK_COLOUR) * 4));
      
      Uart_Print(newValue, 18);
      break;

    case SIMPLEPROFILE_CHAR9:
      SimpleProfile_GetParameter( SIMPLEPROFILE_CHAR9, newValue );      
      osal_memcpy(&UUID9Recive, newValue, sizeof(UUID9Recive));
      get_task_colour_user();
      
      osal_set_event(simpleBLEPeripheral_TaskID, SBP_SET_PWM_EVT);
      Uart_Print(newValue, 20);
      break;      
...
uint8 GetSimpleBLEPeripheral_TaskID(void)
{
